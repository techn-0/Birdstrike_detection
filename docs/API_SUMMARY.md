...
| 위험도 | 조건 | 의미 |
|--------|------|------|
| `green` | 조류 미탐지 (bird_count = 0) | 정상 상태 |
| `yellow` | 1개 조류 + 신뢰도 < 30% | 모니터링 대상 |
| `orange` | 1개 조류 + 신뢰도 ≥ 30% | 주의 필요 |
| `red` | 2개 이상 조류 탐지 | 즉시 경보 필요 |

위험도는 `DetectionService.calculate_risk_by_count_and_confidence()` 메서드에서 자동 계산됩니다.
