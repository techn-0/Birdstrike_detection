# 🚁 AirGuardNet - 공항 조류 탐지 시스템

## 📋 프로젝트 개요

**AirGuardNet**는 공항 안전을 위한 실시간 조류 탐지 및 모니터링 시스템입니다. YOLO 기반 조류 탐지 모델의 결과를 받아 웹 인터페이스에서 실시간으로 시각화하고, 위험도에 따른 경보 시스템을 제공합니다.
...
새로운 조류 탐지 수량과 신뢰도 기반 위험도 분류:

| 위험도 | 색상 | 조건 | 의미 |
|--------|------|------|------|
| 🔴 **red** | 빨강 | 2개 이상 조류 탐지 | 즉시 경보 필요 |
| 🟠 **orange** | 주황 | 1개 조류 + 신뢰도 ≥ 30% | 주의 필요 |
| 🟡 **yellow** | 노랑 | 1개 조류 + 신뢰도 < 30% | 모니터링 대상 |
| 🟢 **green** | 초록 | 조류 미탐지 | 정상 상태 |
